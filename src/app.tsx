				...
				{state.w.map(([id, word]) => {
					...
					return (
						<button
							...
							key={id}
							className={twJoin(
								"rounded-sm border bg-amber-50/80 px-7 py-6 text-lg font-bold text-neutral-950 uppercase transition-all duration-300 disabled:scale-50 disabled:border-transparent disabled:bg-transparent disabled:text-neutral-600",
				...
				return (
					...
					>
						{wordIds.map((wordId) => {
							...
							return (
								<div
									className="rounded-sm bg-neutral-800 px-3 py-2 text-sm text-white uppercase"
									key={collectionId}
								>
									{word}
								</div>
						...
						<button
							type="button"
							className="ms-4 rounded-sm border border-neutral-600 p-3 text-sm/1"
