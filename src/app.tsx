						...
						<div className="col-start-1 col-end-8 flex flex-wrap gap-1">
							{wordIds.map((wordId) => {
								...
								return (
									<div
										className={twJoin(
											"flex cursor-grab items-center gap-3 rounded-sm bg-neutral-900 px-3 py-2 text-center text-sm text-white uppercase",
			...
			{words.map(([id, word]) => {
				...
				return (
					<button
						...
						key={id}
						className={twJoin(
							"relative col-span-2 rounded-sm border bg-amber-50/80 px-2 py-6 text-lg font-bold text-neutral-950 uppercase transition-colors duration-300",
...
function DragWord() {
	...
	return (
		<div
			className="absolute rounded-sm bg-neutral-900 px-3 py-4 text-center text-sm/1 text-white uppercase opacity-0 shadow-sm transition-opacity"
			ref={draggingRef}
		>
